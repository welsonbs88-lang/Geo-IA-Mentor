...
[![Status](https://img.shields.io/badge/status-experimental-yellow)](https://github.com/welsonbs88-lang/Geo-IA-Mentor) [![Python](https://img.shields.io/badge/python-3.8%2B-blue)](https://www.python.org/) [![License](https://img.shields.io/badge/license-MIT-green)](LICENSE)
...
- [Como Executar](#como-executar-️)
- [Exemplos de Interação](#exemplos-de-interação-)
- [Arquitetura (visão geral)](#arquitetura-visão-geral-)
- [Memória Conversacional e Sessões](#memória-conversacional-e-sessões-)
- [Melhorias e Produção](#melhorias-e-produção-)
...
O script realiza uma simulação de conversa com perguntas pré-definidas. Para uso interativo ou integração via API, adapte o ponto de entrada conforme a sua aplicação.
...
Recomendações: para produção, troque `InMemory` por armazenamento persistente (Redis, banco de dados). Defina políticas de retenção e anonimização para proteger dados sensíveis.
...
1. Abra issues para bugs ou ideias;  
2. Envie PRs com melhorias de código ou documentação;  
3. Documente fluxos desejados (ex.: persistência, autenticação).

Fluxo sugerido: Fork → criar branch com nome descritivo → commits claros → abrir PR. Descreva mudanças e motivos no PR.
